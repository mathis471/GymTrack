...
function normalize(s){
  ...
  n.plans=n.plans.map(p=>{const ids=Array.isArray(p.exerciseIds)?p.exerciseIds.filter(id=>n.library.some(e=>e.id===id)):[];const sets={...(p.exerciseSets||{})};ids.forEach(id=>{if(!sets[id]){const e=n.library.find(x=>x.id===id);sets[id]=e?.defaultSets||3;}});return {...p,id:p.id||uid(),name:String(p.name||"Training"),exerciseIds:ids,exerciseSets:sets};});
...
function workoutTemplate(e,plannedSets){const previous=state.workouts.filter(w=>w.items?.some(i=>i.exerciseId===e.id)).sort((a,b)=>new Date(a.date)-new Date(b.date)).at(-1)?.items.find(i=>i.exerciseId===e.id);const prevSets=previous?.sets||[];return Array.from({length:Math.max(Number(plannedSets)||e.defaultSets||3,prevSets.length||0)},(_,i)=>({value:prevSets[i]?.value??(e.unit==="bodyweight"?"BW":""),reps:prevSets[i]?.reps??e.targetReps??"",done:false}))}
...
async function confirmStartWorkout(){
 ...
 const p=activePlan();if(!p)return;
 state.activeWorkout={id:uid(),planId:p.id,workoutDate:date,startedAt:new Date().toISOString(),collapsedExerciseIds:[],items:p.exerciseIds.map(id=>({exerciseId:id,sets:workoutTemplate(ex(id),p.exerciseSets?.[id])}))};
...
function newPlan(){openModal("Neuer Trainingsplan",`<div class="form-grid"><div><label class="label">Name</label><input id="planName" class="field" placeholder="z. B. Pull"></div><div class="section-head no-margin"><div><h3>Übungen</h3><p class="muted small">Du kannst direkt Übungen auswählen oder eine neue Übung anlegen.</p></div></div><div class="multi-exercise-list">${state.library.map(e=>`<label class="multi-choice"><input type="checkbox" class="new-plan-exercise" value="${e.id}"><span><b>${esc(e.name)}</b><small>${e.defaultSets} Sätze · ${unitLabel(e.unit)}</small></span></label>`).join("")}</div><button class="secondary" onclick="newExerciseFromPlan()">＋ Neue Übung erstellen</button><button class="primary" onclick="savePlan()">Plan erstellen</button></div>`)}
async function savePlan(){const n=$("planName").value.trim();if(!n){toast("Bitte einen Namen eingeben.");return}const ids=[...document.querySelectorAll(".new-plan-exercise:checked")].map(x=>x.value);const p={id:uid(),name:n,exerciseIds:ids,exerciseSets:{}};ids.forEach(id=>p.exerciseSets[id]=ex(id)?.defaultSets||3);state.plans.push(p);state.activePlanId=p.id;await save();closeModal();render()}
function newExerciseFromPlan(){openModal("Neue Übung",exerciseForm(null,"newPlan"))}
function editPlan(id){const p=state.plans.find(x=>x.id===id);if(!p)return;const rows=p.exerciseIds.map(eid=>{const e=ex(eid);return e?`<div class="library-row"><div><b>${esc(e.name)}</b><div class="exercise-meta">${unitLabel(e.unit)}</div></div><div class="two plan-set-control"><div><label class="label">Sätze</label><input id="planSets-${eid}" class="field" type="number" min="1" max="20" value="${Number(p.exerciseSets?.[eid]||e.defaultSets||3)}"></div><button class="secondary danger" onclick="removeFromPlan('${p.id}','${eid}')">Entfernen</button></div></div>`:``}).join("");openModal("Plan bearbeiten",`<div class="form-grid"><div><label class="label">Name</label><input id="planName" class="field" value="${esc(p.name)}"></div><div><h3>Übungen & Sätze</h3>${rows||`<div class="empty compact">Noch keine Übungen im Plan.</div>`}</div><button class="primary" onclick="savePlanEdit('${p.id}')">Änderungen speichern</button><button class="secondary" onclick="addToPlan('${p.id}')">＋ Übung hinzufügen</button>${state.plans.length>1?`<button class="secondary danger" onclick="deletePlan('${p.id}')">Plan löschen</button>`:""}</div>`)}
async function savePlanEdit(id){const p=state.plans.find(x=>x.id===id);if(!p)return;const n=$("planName").value.trim();if(n)p.name=n;p.exerciseSets=p.exerciseSets||{};p.exerciseIds.forEach(eid=>{const v=Number($("planSets-"+eid)?.value);if(Number.isFinite(v)&&v>=1)p.exerciseSets[eid]=Math.min(20,Math.round(v));});await save();closeModal();render();toast("Plan gespeichert")}
...
async function attachSelected(pid){
  ...
  ids.forEach(eid=>{if(!p.exerciseIds.includes(eid)){p.exerciseIds.push(eid);p.exerciseSets=p.exerciseSets||{};p.exerciseSets[eid]=ex(eid)?.defaultSets||3;}});
...
async function removeFromPlan(pid,eid){const p=state.plans.find(p=>p.id===pid);if(!p)return;p.exerciseIds=p.exerciseIds.filter(x=>x!==eid);if(p.exerciseSets)delete p.exerciseSets[eid];await save();closeModal();editPlan(pid);render()}
function quickAdd(){
  if(currentTab==="plan"){
