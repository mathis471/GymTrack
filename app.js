...
async function startWorkout(){if(state.activeWorkout){setTab("workout");return}const p=activePlan();if(!p)return;state.activeWorkout={id:uid(),planId:p.id,startedAt:new Date().toISOString(),items:p.exerciseIds.map(id=>({exerciseId:id,sets:workoutTemplate(ex(id))}))};await save();setTab("workout")}
...
function addSet(i){state.activeWorkout.items[i].sets.push({value:"",reps:"",done:false});save();render()}
